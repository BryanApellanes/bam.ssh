...
| 9 | SFTP: protocol v3 (OpenSSH-compatible), client + server, virtual filesystem | **Complete** — SFTP v3 protocol engine in `bam.ssh.sftp` (framing over an `ISftpChannel`, ATTRS/OPEN/READ/WRITE/OPENDIR/READDIR/STAT/SETSTAT/MKDIR/RMDIR/REMOVE/RENAME/REALPATH), `SftpClient` (single-reader id correlation, `UploadAsync`/`DownloadAsync`/`ListDirectoryAsync`), `SftpServer` engine + `ISftpFileSystem` with `InMemorySftpFileSystem` and path-jailed `PhysicalSftpFileSystem`; `MapSftp` subsystem glue in `bam.ssh.server` (D-016); 5 tests incl. crown-jewel `SftpClient` over the production `SshClient`↔`SshServer` doing full round-trips against both filesystems, walkthrough in [docs/protocol/sftp.md](protocol/sftp.md) |
| 10 | SCP: upload/download, recursive | **Complete** — SCP `scp1` binary protocol in `bam.ssh.scp` (`Bam.Ssh.Scp`): symmetric `ScpSender`/`ScpReceiver` role engines over a one-method `IScpChannel` seam, `ScpProtocol` (acks, control lines, exact-length data), `ScpControlMessage` (C/D/E/T parse+format) and `ScpCommand` (scp command-line parse); `ScpClient` (`UploadAsync`/`DownloadAsync`, single files + recursive trees, one exec channel per transfer); server `MapScp(ISftpFileSystem)` exec handler in `bam.ssh.server`; reuses the Phase 9 `ISftpFileSystem` store on both ends (D-017); 5 tests incl. crown-jewel `ScpClient` over the production `SshClient`↔`SshServer` doing single-file + recursive-tree round-trips against in-memory and path-jailed physical filesystems, walkthrough in [docs/protocol/scp.md](protocol/scp.md) |
| 11 | Port forwarding: local, remote, dynamic SOCKS | **Complete** — RFC 4254 §7 TCP/IP forwarding in `bam.ssh.forwarding` (`Bam.Ssh.Forwarding`): four wire codecs (`DirectTcpIpChannelRequest`/`ForwardedTcpIpChannelInfo`/`TcpIpForwardRequest` + bound-port reply), a fault-isolated `SshChannelSocketBridge`, client forwarders `LocalPortForwarder`/`DynamicPortForwarder`/`RemotePortForwarder` (SOCKS5 no-auth + SOCKS4/4a), and reusable acceptor engines `DirectTcpIpChannelAcceptor` (connect side) + `TcpIpForwardListener` (listen side); connection-layer seams `RegisterChannelOpenHandler`/`ISshGlobalRequestHandler`/`SendGlobalRequestWithReplyAsync`; server `EnableTcpForwarding` + `SshServerForwarding`; `SshClient.Forward{Local,Remote,Dynamic}PortAsync` (D-018); 5 tests incl. crown-jewel `-L`/`-R`/`-D` round-trips over the production stack against real TCP echo targets, walkthrough in [docs/protocol/port-forwarding.md](protocol/port-forwarding.md) |
| 12 | Performance: benchmarks, allocation elimination, pipelines tuning | **Complete** — dependency-free benchmark harness in `bam.ssh.benchmarks` (`BenchmarkRunner`/`BenchmarkResult`, no BenchmarkDotNet) with packet-layer and end-to-end channel scenarios; eliminated the per-packet, payload-proportional allocations it surfaced — `ChaCha20Poly1305Cipher` (reuse engines/MAC/key-params + span Poly1305, **69,264→952 B/op at 32 KiB, ~73×**), `AesCtrCipher` (batch 64 ECB blocks/call + reused HMAC, **361,365→5,928 B/op ~61× and ~11× faster**), `SshPacketWriter` (reuse `PooledBufferWriter`, 64→32 B/op); all behavior-preserving (118/118 tests pass). Documented finding: the default chacha20-poly1305-first preference caps end-to-end throughput ~61 MiB/s vs aes256-gcm's ~540 MiB/s AES-NI path — left as a configurable trade-off (D-019). Walkthrough in [docs/protocol/performance.md](protocol/performance.md) |
| 13 | Documentation: architecture, protocol walkthrough, diagrams, API reference, guides | **Complete** — `docs/README.md` documentation hub (rendered layering + connect→auth→exec sequence Mermaid diagrams, the 12-walkthrough index with reading order, project map, tests/benchmarks pointers), `docs/guides/getting-started.md` (real-API recipes: client/host-key trust/auth/shell/SFTP/SCP/forwarding/server), and a polished top-level `README.md` (accurate API sketches, forwarding added, feature-complete status, documentation index) (D-020). **This completes the 13-phase stack.** |
...
| 4254 §7 | TCP/IP Port Forwarding | forwarding |
| RFC 1928 / 1929 | SOCKS5 / SOCKS4a (dynamic forwarding) | forwarding |
